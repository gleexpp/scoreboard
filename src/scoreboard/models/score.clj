...
(defn get-institute-final-score []
  (let [scores (get-player-final-scores)
        college-scores (group-by #(nth % 1) scores)
        colleges (keys college-scores)]
    (for [i (range (count colleges))]
      (into (into [(nth colleges i)] (sum-of-vecs
                                     (map #(subvec % 2) (college-scores (nth colleges i)))))
            [(str (inc i))]))))